...
class Helper {
	...
	public static function upload_photo($picture, $path, $file = null, $company_id = null)
	{
		if($file == null) {
			$file_name = time();
			$file_name .= rand();
	...
	public static function voice_access_token($companyId, $outgoing_caller_id, $To, $device, $user) {
		$settings = json_decode(json_encode(Setting::where('company_id',$companyId)->first()->settings_data));
		$siteConfig = $settings->site;

		$twilioAccountSid = $siteConfig->sms_account_sid;
		$twilioApiKey = $siteConfig->twilio_app_key;
		$twilioApiSecret = $siteConfig->twilio_app_secret;
		$outgoingApplicationSid = $siteConfig->twilio_app_sid;
		$fcmCredentialSid = $siteConfig->twilio_fcm_sid;

		if($device == 'ios'){
			if($user == 'provider'){
				$fcmCredentialSid = $siteConfig->twilio_ios_provider_sid;
			}
			else{
				$fcmCredentialSid = $siteConfig->twilio_ios_user_sid;
			}
		}
		...
		$voiceGrant->setPushCredentialSid($fcmCredentialSid);
		...
		// $accountSid = $siteConfig->sms_account_sid;
  //       $apiKey = $siteConfig->twilio_app_key;
  //       $apiSecret = $siteConfig->twilio_app_secret;
  //       $applicationSid = $siteConfig->twilio_app_sid;
  ...
  //       return $accessToken->toJWT();
	...
	public static function paystack_verify($secret_key,$reference){
	...
	public static function paystack_refund($secret_key,$data){
	...
	public static function paystack_charge_authorization($secret_key,$data){
		$url = "https://api.paystack.co/transaction/charge_authorization";
		$fields = [
			'authorization_code' => $data['card'],
			'email' => $data['email'],
			'amount' => $data['amount']
		];
	...
	public static function send_ride_invoice($payment){
		try{
			$ride_request = $payment->ride_request;

			$settings = json_decode(json_encode(Setting::where('company_id',$ride_request->user->company_id)->first()->settings_data));

			Mail::send('mails.ride_invoice', ['payment' => $payment, 'settings' => $settings], function ($mail) use ($ride_request, $settings){
				$mail->from($settings->site->mail_from_address, $settings->site->mail_from_name);
				$mail->to($ride_request->user->email, $ride_request->user->first_name.' '.$ride_request->user->last_name)->subject('[FareRun] Ride Invoice');
			});

			// return view('mails.ride_invoice', ['payment' => $payment, 'settings' => $settings]);
