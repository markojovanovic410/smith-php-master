...
class ReferralResource {
    ...
    */
    ...
    public function create_referral($referral_code, $referral_data, $settings, $referrer)
    {
        ...
        $referral_user=User::where('referral_unique_id',$referral_code)->where('company_id', $referral_data->company_id)->first();
        $type=1;
        if(empty($referral_user)){
            $type=2;
            $referral_user=Provider::where('referral_unique_id',$referral_code)->where('company_id', $referral_data->company_id)->first();
        }
