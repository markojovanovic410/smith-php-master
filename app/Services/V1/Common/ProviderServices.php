	...
	public function checkRequest($request) {
		...
		try{
			...
			if($provider != null) {
				...
				$not_status = ['CANCELLED', 'SCHEDULED', 'PENDING', 'PROVIDEREJECTED'];
				$IncomingRequests = RequestFilter::with(['request.user', 'request.service', 'request.service', 'request'])
				->whereHas('request', function($query) use($not_status){
					$query->whereNotIn('status', $not_status);
			...
			\Log::info("Provider Incoming Request-----------");
		...
		try {        
			...
			try {
				if($request->admin_service == "TRANSPORT" ) {
					...
					$outgoing_caller_id = 'provider-'.$newRequest->id;
					$To = 'user-'.$newRequest->id;
					$provider_token = Helper::voice_access_token($newRequest->company_id, $outgoing_caller_id, $To, $device, 'provider');
					$twilio_voice_token = [
						'user' => json_decode($newRequest->twilio_voice_token)->user,
						'provider' => $provider_token

					];
					$newRequest->twilio_voice_token = json_encode($twilio_voice_token);
					$newRequest->save();
					// Twilio Call End
			...
			if($request->admin_service == "TRANSPORT") {
				...
				if($newRequest->payment_mode != 'CASH'){
					$newRequest_payment = \App\Models\Transport\RideRequestPayment::where('ride_request_id', $newRequest->id)->first();
					$newRequest_payment->provider_id = $this->user->id;
					$newRequest_payment->save();
				}
			}
			...
			if($newRequest->schedule_at != ""){
				...
				if($request->admin_service == "SERVICE" ) {
					...
					$newRequest->started_at = (Carbon::now())->toDateTimeString();
					$newRequest->save();
					//Send message to socket
