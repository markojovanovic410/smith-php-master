...
use App\Models\Common\DriverPromotionUsage;
...
use App\Models\Transport\RideRequestPaymentRefund;
...
class Ride { 
	...
	*/
	...
	public function createRide(Request $request) {
		...
		\Log::info("Available Providers in Send Request-------------------");
		\Log::info($Providers);
		\Log::info("-------------------");
		...
		try {
			...
			if($request->payment_mode == 'PAYSTACK' || $request->payment_mode == 'CARD'){
			...
			if($rideRequest->status == 'SEARCHING'){
				...
				$outgoing_caller_id = 'user-'.$rideRequest->id;
				$To = 'provider-'.$rideRequest->id;
				$user_token = Helper::voice_access_token($rideRequest->company_id, $outgoing_caller_id, $To, $device, 'user');

				$twilio_voice_token = ['user' => $user_token];
				$rideRequest->twilio_voice_token = json_encode($twilio_voice_token);
				$rideRequest->save();
			}
	...
	public function cancelRide($request) {
		...
		try{
			...
			if(in_array($rideRequest->status, ['SEARCHING','STARTED','ARRIVED','SCHEDULED', 'PENDING'])) {
				...
				if($rideRequest->payment_mode != 'CASH' && $rideRequest->paid == true && !empty($payment))
				{
					...
					if($request->cancelled_by == "USER" && $rideRequest->started_at) {
						$time_diff = Carbon::now()->diffInSeconds(Carbon::parse($rideRequest->started_at));

						// If user cancel after 10 mints from provider accepted the ride
						if(($time_diff/60) >= 10){
							$deducted_amount_10 = (10 * $payment->total) / 100;
							$amount = $payment->total - $deducted_amount_10;
							$data = [
					...
					$refund = Helper::paystack_refund($secret_key, $data);

					if($refund->status && array_key_exists("data",$refund)){
						$payment->is_refunded = true;
						$payment->save();

						$ride_payment_refund = RideRequestPaymentRefund::updateOrCreate(
							['ride_request_id' => $rideRequest->id],
							[
								'response_status' => $refund->status, 
								'message' => $refund->message,
								'reference' => $payment->payment_id,
								'amount' => ($refund->data->amount) ? $refund->data->amount /100 : 0,
								'currency' => $refund->data->currency,
								'deducted_amount' => $refund->data->deducted_amount,
								'refund_status' => $refund->data->status,
								'refunded_by' => $refund->data->refunded_by,
								'domain' => $refund->data->domain,
								'fully_deducted' => $refund->data->fully_deducted,
								'data_json' => json_encode($refund),
							]
						);
					}
					else{
	...
	public function updateRide(Request $request) { 
		try{
			...
			if($request->status == 'DROPPED' && $rideRequest->payment_mode != 'CASH') {
				...
				if ($rideRequest->paid == 1) {
					(new \App\Http\Controllers\V1\Transport\Provider\TripController)->callTransaction($rideRequest->id);
				}
			...
			if($request->status == 'DROPPED') {
				...
				// Driver Promotions Start
				$promos = DriverPromotion::where('company_id', $this->company_id)
										->whereDoesntHave('usage', function($query) use($rideRequest){
											$query->where('provider_id', $rideRequest->provider_id);
										})
				...
				foreach ($promos as $promo) {
					$rides_count = RideRequest::where('provider_id', $rideRequest->provider_id)
												->where('status', 'COMPLETED')
												->whereBetween('created_at', [Carbon::parse($promo->start), Carbon::parse($promo->end)])
												->count();
					if($rides_count >= $promo->no_of_rides){
						$p = Provider::find($rideRequest->provider_id);
						$p->wallet_balance = $p->wallet_balance + $promo->bonus;
						$p->save();

						$usage = new DriverPromotionUsage();
						$usage->company_id = $this->company_id;
						$usage->provider_id = $p->id;
						$usage->promocode_id = $promo->id;
						$usage->status = 'USED';
						$usage->save();
					}
				}
				// Driver Promotions End
			}
		...
		try {  
			...
			if($rideRequest->promocode_id>0){
				if($Promocode = Promocode::find($rideRequest->promocode_id)){
					...
					$PromocodeUsage->company_id = $rideRequest->company_id;
			...
			$Payment->company_id = $rideRequest->company_id;
			...
			if(!empty($rideRequest->admin_id)){
				$Fleet = Admin::where('id',$rideRequest->admin_id)->where('type','FLEET')->where('company_id', $rideRequest->company_id)->first();
			...
			$start_time_check = PeakHour::where('start_time', '<=', $start_time)
											...
											->where('company_id', $rideRequest->company_id)
											->first();

			if($start_time_check){

				$RideCityPriceList = RideCityPrice::where('geofence_id',$rideRequest->geofence_id)->where('ride_delivery_vehicle_id',$rideRequest->ride_delivery_id)->where('company_id', $rideRequest->company_id)->first();
			...
			$data = Helper::send_ride_invoice($Payment);
