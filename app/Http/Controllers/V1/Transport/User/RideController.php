...
class RideController extends Controller
{
	...
	public function estimate(Request $request)
	{
		...
		try{
			RideRequest::where('status', 'PENDING')->where('user_id', Auth::guard('user')->user()->id)->delete();
			UserRequest::where('status', 'PENDING')->where('user_id', Auth::guard('user')->user()->id)->delete();
			...
			if($geofence) {
				...
				if( Auth::guard(strtolower(Helper::getGuard()))->user() != null ) {
					$is_direct_applied = false;
					$promocodes = Promocode::where('company_id', $this->company_id)
											->where('service', 'TRANSPORT')
											->where('start','<=',date("Y-m-d H:i"))
				        					->where('end','>=',date("Y-m-d H:i"));
				    $no_of_rides = RideRequest::where('user_id', Auth::guard('user')->user()->id)->where('status', 'COMPLETED')->count();
					$new_user = true;
					if($no_of_rides >= 5){
						$new_user = false;
					}
					if(!$new_user){
        				$promocodes = $promocodes->where('user_type', 'ALL')->get();
        			}
        			else{
        				$promocodes = $promocodes->get();
        			}
					foreach ($promocodes as $key => $promo) {
        				$usage_count = PromocodeUsage::where('status', 'USED')
        										->where('created_type', 'USER')
        										->where('promocode_id', $promo->id)
        										->count();
        				if($usage_count >= $promo->no_of_rides){
        					$promocodes->forget($key);
        				}
        			}

					// Check Direct Apply Promocodes
					$direct_promocodes = $promocodes->where('apply_type', 'DIRECT')->values();
					$direct_promocodes = $direct_promocodes->sortByDesc('percentage')->first();
					if($direct_promocodes){
						$a_fare = $fare->estimated_fare;
						$discount_amount = ($fare->estimated_fare * ($direct_promocodes->percentage/100));
						if($discount_amount>$direct_promocodes->max_amount){
							$Discount = $direct_promocodes->max_amount;
						}
						else{
							$Discount = $discount_amount;
						}

						$fare->estimated_fare = $fare->estimated_fare - $Discount;
						$fare->promocode_id = $direct_promocodes->id;
						$fare->actual_estimated_fare = $a_fare;
						$fare->discount = true;
						$is_direct_applied = true;
						$promocodes = [];
					}

					// Check Promocodes
				    if(!$is_direct_applied){
				    	$promocodes = $promocodes->where('apply_type', 'PROMOCODE')->values();
				    }
				...
				\Log::info(trans('user.ride.service_not_available_location'));

				return Helper::getResponse(['status' => 500, 'message' => trans('user.ride.service_not_available_location'), 'error' => trans('user.ride.service_not_available_location')]);
	        }
	...
	{
		\Log::info("User Check Request Status--------------");
		try{

			$check_status = ['CANCELLED', 'SCHEDULED', 'PENDING'];
			...
			\Log::info("Check Request data----------------");
			\Log::info($rideRequest);
			\Log::info("--------------------");
   ...
   public function statement_user(Request $request)
   {
	...
	public function payment(Request $request) {
		...
		try {
			...
			if(empty($payment) && ($UserRequest->payment_mode == 'PAYSTACK' || $UserRequest->payment_mode == 'CARD')){
				$payment = (new Ride())->invoice($UserRequest->id, 0);
			}

			$ride = (new UserServices())->payment($request, $UserRequest, $payment);

			$user_request = UserRequest::where('request_id', $UserRequest->id)->where('admin_service', 'TRANSPORT')->first();
			if(!empty($user_request)){
				$user_request->request_data = json_encode($UserRequest);
				$user_request->save();
			}

			if($ride['status'] == 'error'){
				return Helper::getResponse(['message' => $ride['message'], 'status' => 400]);
			}
			else{
				return Helper::getResponse(['message' => $ride['message']]);
			}
