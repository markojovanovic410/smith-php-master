...
class CommonController extends Controller
{
    ...
    public function driver_data_cities(Request $request) {
        $data = DriverDataCity::orderBy('name');
    ...
    public function type_of_cars(Request $request){
        $data = DriverDataTypeOfCar::orderBy('name')->get();
        return Helper::getResponse(['data' => $data]);
   }
    ...
    public function driverData(Request $request){
        ...
        return Helper::getResponse(['message' => 'Successfully']);
    ...
    public function cmsfaq(Request $request) {
        $faq = CmsFaq::where('company_id', $request->company_id)->get();
        return Helper::getResponse(['data' => $faq]);
    }
    public function cmshowitworks(Request $request) {
        $how = CmsHowItWork::where('company_id', $request->company_id)->orderBy('step', 'asc')->get();
        return Helper::getResponse(['data' => $how]);
    }

    public function home_news(Request $request, $id) {
        $data = News::findOrFail($id);
        return Helper::getResponse(['data' => $data]);
    }

    public function cmshomenews(Request $request) {
        if(isset($request->id) && !empty($request->id)){
            $news = CmsHomeNews::where('company_id', $request->company_id)->where('id', $request->id)->first();    
        }
        else{
            $news = CmsHomeNews::where('company_id', $request->company_id)->orderBy('created_at', 'desc')->get();
        }
    ...
    public function getCities() {
        ...
        $data = CompanyCity::with('city')->where('company_id', 1)->where('country_id', 101)->get();
