    ...
    public function orderpayment($totalAmount,$request){
        ...
        $settings = json_decode(json_encode(Setting::where('company_id', $this->company_id)->first()->settings_data));
              ...
              $card = 0;
                ...
                $payObject = array_values(array_filter( $paymentConfig, function ($e) { return $e['name'] == 'paystack'; }));
                 $paystack = 0;
                ...
                if(count($payObject) > 0) { 
                    $card = $payObject[0]['status'];

                    $SecretObject = array_values(array_filter( $payObject[0]['credentials'], function ($e) { return $e['name'] == 'secret_key'; }));
                    $PublishableObject = array_values(array_filter( $payObject[0]['credentials'], function ($e) { return $e['name'] == 'public_key'; }));
